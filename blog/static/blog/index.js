  ...
  fetch(`/bookmark/${post_id}`, {
    ...
    .then((result) => {
      ...
      if (result.bookmark === "True") {
        ...
        document.querySelector(`#bookmark-${post_id}`).title =
          "You have bookmarked this post";
        document
          .querySelector(`#bookmark-${post_id}`)
  ...
  fetch(`/like/${post_id}`, {
    ...
    .then((result) => {
      if (result.like === "True") {
        ...
        document.querySelector(`#like-${post_id}`).title =
...
function edit_post(edit_button) {
  ...
  save_button.addEventListener("click", () => {
    ...
    const edited_content = document.querySelector(
    ...
    fetch(`/edit/${post_id}`, {
      ...
      .then((result) => {
        ...
        document.querySelector(`#edited-content-${post_id}`).style.display =
          "none";
        ...
        document.querySelector(`#original-content-${post_id}`).style.display =
          "block";
        ...
        document.querySelector(`#original-content-${post_id}`).innerHTML =
          edited_content_html;
      });
  });
}
...
function reply_comment(reply_button) {
  ...
  comment_div.insertAdjacentHTML(
    "afterend",
    '<form id="newForm" class="form-insert py-2 ms-4" method="post"> \
            <div class="d-flex justify-content-between"><h4>Reply:</h4><div><button type="button" class="btn btn-outline-secondary mb-2"  onclick=formExit("' +
      node_id +
